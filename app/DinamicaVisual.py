##Programa DinamicaVisual para automatizacao visual do Gromacs.
## Coordenadores: Fernando Berton Zanchi (Fiocruz-RO) e Rafael Andrade Caceres (UFCSPA-RS)
...
def executa(button):
    selecao_arquivo = aplica.get_object("selecao_arquivo")
    pasta = selecao_arquivo.get_current_folder()
    arquivo = selecao_arquivo.get_filename()
    nome_arquivo, extensao = arquivo.split('.')


    combo_campo_forca = aplica.get_object("campo_forca")
    campo_forca = combo_campo_forca.get_active_id()

    combo_modelo_agua = aplica.get_object("modelo_agua")
    modelo_agua = combo_modelo_agua.get_active_id()

    combo_tipo_caixa = aplica.get_object("tipo_caixa")
    tipo_caixa = combo_tipo_caixa.get_active_id()

    spin_distancia_caixa = aplica.get_object("distancia_caixa")
    distancia_caixa = spin_distancia_caixa.get_value()

    check_neutralize=aplica.get_object("check_neutralize")
    neutralizar_sistema = check_neutralize.get_active()
...
aplica = Gtk.Builder()
aplica.add_from_file("DinamicaVisual.glade")
handlers = {
    "Terminar_aplicacao": Gtk.main_quit,
    "evento_executa": executa
}

aplica.connect_signals(handlers)
janela = aplica.get_object("Aplicacao")
janela.show_all()
